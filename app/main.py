...
from app.db import init_db, get_conn  # DB bootstrap + connection
from app.auth import (                 # Cookie/session + user helpers
...
# Stable session secret (set SECRET_KEY in env for production)
...
def home(request: Request, user_id: Optional[int] = Depends(get_current_user_id)) -> HTMLResponse:
    ...
    return templates.TemplateResponse(
        "index.html",
        {"request": request, "user": user_id, "title": "Home"},
    )
...
def public_trees(request: Request, user_id: Optional[int] = Depends(get_current_user_id)) -> HTMLResponse:
    ...
    # Reuse your dashboard template as a simple list, or make a dedicated template
    return templates.TemplateResponse(
        "dashboard.html",
        {"request": request, "user": user_id, "public_trees": trees, "title": "Public Trees"},
    )
...
def view_tree(
    ...
    # Allow public viewing when is_public=1; otherwise check auth
    if not tree.get("is_public", 0):
        if user_id is None:
            return RedirectResponse(
                "/login?next=" + urllib.parse.quote(f"/tree/{tree_id}"),
    ...
    return templates.TemplateResponse(
        "tree.html",
        {"request": request, "user": user_id, "tree": tree, "title": tree["name"]},
    )
...
async def create_tree(request: Request, user_id: int = Depends(require_auth_user_id)) -> RedirectResponse:
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO trees(name, description, owner_id, is_public) VALUES(?,?,?,?)",
        (name, desc, user_id, 0),
    )
    conn.commit()
    tree_id = cur.lastrowid if "lastrowid" in dir(cur) else None
    if tree_id is None:
        # For Postgres, fetch inserted id
...
async def request_code(request: Request) -> HTMLResponse:
    ...
    if not email:
        return templates.TemplateResponse(
            "login.html",
            {"request": request, "error": "Please enter your email.", "next": next_url},
        )
    ...
    # Send via Resend (configured in app/utils.py)
    try:
        send_email_code(email, code)
    except Exception as e:
        return templates.TemplateResponse(
            "login.html",
            {"request": request, "error": f"Failed to send email: {e}", "next": next_url},
        )

    return templates.TemplateResponse(
        "verify.html",
        {"request": request, "email": email, "next": next_url},
    )
...
async def verify_code(request: Request) -> RedirectResponse | HTMLResponse:
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT code, expires_at FROM login_codes WHERE email=? ORDER BY id DESC LIMIT 1",
        (email,),
    )
    row = cur.fetchone()
    conn.close()
    ...
    if not row:
        return templates.TemplateResponse(
            "verify.html",
            {"request": request, "email": email, "error": "No code requested for this email.", "next": next_url},
        )

    if row["code"] != code:
        return templates.TemplateResponse(
            "verify.html",
            {"request": request, "email": email, "error": "Invalid code.", "next": next_url},
        )

    try:
        expires = datetime.fromisoformat(row["expires_at"])
    except Exception:
        # If DB driver returns timezone-aware format, you may need fromisoformat replacement.
        expires = datetime.utcnow() - timedelta(days=1)

    if expires < datetime.utcnow():
        return templates.TemplateResponse(
            "verify.html",
            {"request": request, "email": email, "error": "Code expired. Please request a new one.", "next": next_url},
        )
