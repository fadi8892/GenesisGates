// Genesis Gates — GEDCOM Pro v2: GEDCOM confirm flows, fixed map (Leaflet CSS), geocoding + cache, Family Circles diagram
(function(){
  ...
  let state = Object.assign({
    ...
    geoCache: {}       // { "City, Country": {lat:x, lon:y} }
  }, loadState());
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // export tip
    qs('#btnExportTip').addEventListener('click', ()=> exportGEDCOM());
  ...
  // Bind Overview-specific buttons
  function bindOverview(){
    qs('#btnLocatePlaces').addEventListener('click', async ()=>{
    ...
    qs('#fileImportGED').addEventListener('change', (e)=>{
      if(state.people.length>0){
  ...
  function importJSON(e){
    ...
    r.onload=()=>{ try{ const data=JSON.parse(r.result); state=Object.assign(state,data); saveState(state); renderPeople(); refreshSelectors(); if(window._treeReady) drawGraph(); if(window._mapReady) refreshMap(); }catch{ alert('Invalid JSON'); } };
    r.readAsText(f);
  }

  // --- GEDCOM import/export (basic + RESI support)
  function exportGEDCOM(){
    ...
    // Families: derive from spouses + links
    const fams = [];
  ...
  function importGEDCOM(e){
    ...
    r.onload = ()=>{
      ...
      alert('GEDCOM imported. You can always Export GEDCOM to back up your data.');
    };
    r.readAsText(f);
  }
  ...
  // --- Tree & Circles diagrams (D3)
  let d3Loaded=false, svg, g, zoom, diagramMode='tree'; // 'tree' or 'circles'
  ...
  function drawCircles(data){
    ...
    // click name to collapse (use top label)
  ...
  function focusByName(q){
    ...
    // For simplicity, just fit (so user can see labels), then they can center manually
    fitGraph();
  }
